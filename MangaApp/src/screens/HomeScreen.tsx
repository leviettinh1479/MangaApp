     ...
     <ScrollView showsVerticalScrollIndicator = {false}> 
          ...
          <View style={{flexDirection:'column',justifyContent:'flex-start'}}>
            ...
            <FlatList   style={{ flex: 1,marginBottom:10}}
              ...
              renderItem={({item}) => <ItemManga  onpress={() => navigation.navigate("DetailScreen", {_id: item._id})} image={item.image} nameManga={item.name} nameAuthor={item.author}  description={item.status}  />}
              horizontal
              showsHorizontalScrollIndicator = {false}
