...
import {
  addToFavorites,
  removeFromFavorites,
} from '../components/utils/redux/action';
...
const ItemChaps: React.FC<ItemChap> = ({title, content, onpress}) => {
  return (
...
const DetailScreen: React.FC<ScreenAProps> = ({navigation}) => {
  // chi tiết manga
  ...
  useEffect(() => {
    const getMangaID = async () => {
      try {
        ...
        const respone1 = await AxiosIntance().get(
          '/api/chapter/getall/' + dataId,
        );
        ...
        const respone2 = await AxiosIntance().get(
          '/api/manga/getgenres/' + dataId,
        );
        if (respone2) {
 ...
 // const [isFavorite, setIsFavorite] = useState(false);
  ...
  //   if (favorites.some((favorite) => favorite.manga._id === dataId)) {
  //     // Xóa khỏi danh sách yêu thích
  ...
  //   }
  // };
  const isLike= favorites.some((favorite: { manga: { _id: any; }; }) => favorite.manga._id === dataId)
  const handleToggleFavorite = () => {
    // Find the index of the item with the matching _id
    const indexToRemove = favorites.findIndex((favorite: { manga: { _id: any; }; }) => favorite.manga._id === dataId);
  if (indexToRemove !== -1) {
      dispatch(removeFromFavorites(favorites[indexToRemove]._id));
      console.log('>>>>>RemoveFavorite',favorites[indexToRemove]._id);
    } else {
      // Thêm vào danh sách yêu thích
      dispatch(addToFavorites(dataId, infoUser._id));
 ...
 // const isFavorite = favorites.some((favorite) => favorite.manga._id === dataId);
  return (
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {loading ? (
          ...
          <ImageBackground
            ...
            <Image
              style={{position: 'absolute', height: 250, width: 160}}
              resizeMode="cover"
              source={{uri: GetMangaId.image}}
            />
            <View
              style={{
                backgroundColor: '#FF97A3',
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  navigation.navigate('DetailChap', {
                    _id: GetChapterId[0]._id,
                    chapter: GetMangaId.chapters[0],
                  })
                }>
                <Feather
                  name="book"
        ...
        <View
          ...
          <View
            ...
            <TouchableOpacity onPress={handleToggleFavorite}>
              ...
              { isLike ? <Ionicons name="bookmark" color="#FF97A3" size={20} style={{ padding: 1, marginTop: 5 }} />
               : 
          ...
          <View
            ...
            <View
              style={{width: 1, height: 20, backgroundColor: '#000000'}}></View>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Fontisto
                name="lightbulb"
                color="#000000"
          ...
          <Text
            ...
            interactions and their effecs on productivity and creaiviy.
          </Text>
          <FlatList
